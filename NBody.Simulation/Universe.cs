namespace NBody.Simulation
{
  public class Universe
  {
    ...
    public OcTreeCache OcTreeCache
    {
      get;
      private set;
    } = new OcTreeCache();
    ...
    {
      if (frame++ == trajectoryUpdateFrequency) frame = 0;
    ...
    {
      int required = Bodies.Length * 8;
      if (target.Length < required) throw new ArgumentException();
      int j = 0;
      foreach (var body in Bodies) {
        var loc = body.Location;
        var vel = body.Velocity;
        double mag = vel.Magnitude();
        Vector unit = mag > 0 ? vel / mag : Vector.Zero;
        target[j++] = (float)loc.X;
        target[j++] = (float)loc.Y;
        target[j++] = (float)loc.Z;
        target[j++] = (float)body.Mass;
        target[j++] = (float)unit.X;
        target[j++] = (float)unit.Y;
        target[j++] = (float)unit.Z;
        target[j++] = 0f;
      }
    }
    ...
    private void SetInitialConditions()
    {
      ...
      foreach (var body in Bodies) body.InitTrajectory();
    ...
    private void InitLocations(Random random)
    {
      ...
      i < Bodies.Length;
      i++) {
        Bodies[i] = new Body { Location = RandomInDisk(random, 10), Mass = MassMultiplier };
      }
    }
    ...
    private Vector RandomInDisk(Random random, double radius)
    {
      ...
      return new Vector(r * Math.Sqrt(1.0 - cosTheta * cosTheta) * Math.Cos(phi), r * cosTheta, 0.05 * r * Math.Sqrt(1.0 - cosTheta * cosTheta) * Math.Sin(phi));
    }
    ...
    private OcTree BuildOcTree(double halfWidth)
    {
      ...
      foreach (var body in Bodies) tree.Add(body);
      return tree;
    }
