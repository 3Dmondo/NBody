...
internal class Universe
{
  ...
  public OcTreeCellCache OcTreeCache { get; private set; } = new OcTreeCellCache();
  public OcTreeCell Tree { get; private set; }
  private int frame = 0;
  ...
  public Vector256<double> SimulateLeapFrog()
  {
  ...
  public Vector256<double> SimulateRungeKutta4()
  {
    if (frame++ == trajectoryUpdateFrequency)
      frame = 0;
    Tree = AccelerateBodies();
  ...
  private static void SetBodyVelocity(Body b, Random random, OcTreeCell tree)
  {
    if (b.Position == Vector256<double>.Zero) return;
    var d = tree.CenterOfMass - b.Position;
    var distance = d.Magnitude();
    var dir = b.Acceleration.Cross(Vector256.Create(0.0, 0.0, 1.0, 0.0));
    dir /= dir.Magnitude();
    ...
    b.Velocity += Vector256.Create(0, 0, Vz, 0);
  }
  ...
  private Vector256<double> RandomInDisk(Random random, double radius)
  {
  ...
  private OcTreeCell AccelerateBodies()
  {
    double halfWidth = GetHalfWidth();
    OcTreeCell tree = BuildOcTree(halfWidth);
    Parallel.ForEach(Bodies, b => {
      b.PotentialEnergy = 0;
      b.Acceleration = Vector256<double>.Zero;
      b.Interactions = 0;
      b.TooClose = false;
      tree.Accelerate(b);
    });
  ...
  private OcTreeCell BuildOcTree(double halfWidth)
  {
    OcTreeCache.Count = 0;
    var tree = OcTreeCache.GetNextOcTree(Vector256<double>.Zero, 2.1 * halfWidth);
  ...
  private double GetHalfWidth()
  {
    return Bodies.
      Select(
      b => {
        var abs = Vector256.Abs(b.Position);
